...
contname = 'contname'  # Set your container name here
...
class TestHW2(unittest.TestCase):
    ...
    '''
    Creating a subnet:
        2sudo docker network create --subnet=10.0.0.0/16 mynet
    '''
    ...
    def spin_up_nodes(self):
        ...
        exec_string_main = sudo + \
            " docker run -p 8083:8080 --net=mynet -e IP=10.0.0.20 -e PORT=8080 --ip=10.0.0.20 -d %s" % contname
        ...
        exec_string_forw1 = sudo + \
            " docker run -p 8084:8080 --net=mynet -e IP=10.0.0.21 -e PORT=8080 -e MAINIP=10.0.0.20:8080 --ip=10.0.0.21 -d %s" % contname
        print exec_string_forw1
        self.__class__.node_ids.append(subprocess.check_output(
            exec_string_forw1, shell=True).strip('\n'))

        exec_string_forw2 = sudo + \
            " docker run -p 8085:8080 --net=mynet -e IP=10.0.0.22 -e PORT=8080 -e MAINIP=10.0.0.20:8080 --ip=10.0.0.22 -d %s" % contname
    ...
    def tearDown(self):
        if self.__class__.all_tests_done:
            ...
            shell_command = "docker stop -f $(docker ps -aq)"
            subprocess.check_output(shell_command, shell=True)
